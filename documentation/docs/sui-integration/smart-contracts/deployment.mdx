...
echo "🚀 Starting SUI CLI installation..."
...
echo "🏗️  Installing SUI CLI from source..."
...
echo "✅ Verifying SUI CLI installation..."
...
<div className="card padding--md mt-4" style={{ borderLeft: '4px solid #ff9800' }}>
  <h3><img src="/img/icons/warning.svg" width="20" height="20" style={{ verticalAlign: 'middle', marginRight: '8px' }} />Important Security Note</h3>
  <p>
...
if ! sui client envs | grep -q "testnet"; then
    echo "❌ Testnet environment not configured"
    exit 1
fi
...
if [ "$BALANCE" -lt 100000000 ]; then
    echo "⚠️  Low balance: $BALANCE MIST. Consider getting more tokens from faucet."
fi

echo "✅ Account balance: $BALANCE MIST"

# Validate Move.toml
if ! grep -q "neuranft" Move.toml; then
    echo "❌ Invalid Move.toml configuration"
    exit 1
fi

echo "✅ Move.toml configuration valid"
...
echo "✅ Updated Move.toml with address: $CURRENT_ADDRESS"
  ...
  <TabItem value="validation" label="Build Validation">
    ...
    if [ -d "build/neuranft" ]; then
        ...
        find build/ -name "*.mv" | while read file; do
            echo "📦 Bytecode: $file ($(stat -f%z "$file" 2>/dev/null || stat -c%s "$file") bytes)"
        done
    else
        echo "❌ Build failed"
        exit 1
...
echo "🚀 Starting NeuraLabs contract deployment..."
...
log "✅ Balance check passed: $BALANCE MIST"
...
if [ $? -eq 0 ]; then
    ...
    log "⚠️  Package verification failed - this is normal for newly deployed packages"
fi
...
if [ "$COLLECTION_ID" != "null" ] && [ -n "$COLLECTION_ID" ]; then
    log "✅ Collection created: $COLLECTION_ID"
    ...
    log "⚠️  Collection creation failed or ID not found"
fi
...
log "💾 Environment configuration saved to .env.deployment"
...
║ Time:       $(date)
║ Gas Used:   $(echo "$DEPLOY_OUTPUT" | jq '.effects.gasUsed.computationCost + .effects.gasUsed.storageCost') MIST
╚══════════════════════════════════════════════════╝
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
  ...
  <Card>
    <CardHeader>
      <CardTitle>
        <img src="/img/icons/warning.svg" width="20" height="20" style={{ verticalAlign: 'middle', marginRight: '8px' }} />
        Network Issues
      </CardTitle>
      <CardDescription>
  ...
  <Card>
    <CardHeader>
      <CardTitle>
        <img src="/img/icons/info.svg" width="20" height="20" style={{ verticalAlign: 'middle', marginRight: '8px' }} />
        Address Conflicts
      </CardTitle>
      <CardDescription>
...
if [ "$CURRENT_BALANCE" -gt "$RECOMMENDED_GAS" ]; then
    ...
    echo "⚠️  Low balance. Consider requesting more tokens:"
...
if [ -f "deployment_info.json" ]; then
    ...
    echo "ℹ️  No previous deployment info found"
fi
...
<div className="card padding--md mt-4" style={{ borderLeft: '4px solid #4caf50' }}>
  <h3><img src="/img/icons/success.svg" width="20" height="20" style={{ verticalAlign: 'middle', marginRight: '8px' }} />Deployment Complete!</h3>
  <p>
